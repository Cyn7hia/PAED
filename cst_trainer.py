...
class CustomTrainer(Seq2SeqTrainer):
    ...
    # deprecated
    def compute_loss_(self, model, inputs, return_outputs=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        Subclass and override for custom behavior.
        """
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            if epoch != 0:
                path_model = Path(self.path_model) / f"{self.model_type}_{epoch-1}.pt"
                self.vae.load_state_dict(torch.load(path_model))
                self.train_dataset.vae_sampling(self.vae, k=self.k)
                self.train_dataset.preprocess_fun()
                self.eval_dataset.vae_sampling(self.vae, k=self.k)
                self.eval_dataset.preprocess_fun()
            ...
            elif hasattr(train_dataloader, "dataset") and isinstance(train_dataloader.dataset,
                                                                     IterableDatasetShard):
                train_dataloader.dataset.set_epoch(epoch)

            if is_torch_tpu_available():
                parallel_loader = pl.ParallelLoader(train_dataloader, [args.device]).per_device_loader(
                    args.device)
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss = self.compute_loss(model, inputs, return_outputs=False, eval=True)
