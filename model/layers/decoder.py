...
from model.layers.beam_search import Beam
from model.utils.convert import to_var
from model.utils.vocab import SOS_ID, UNK_ID, EOS_ID
...
class BaseRNNDecoder(nn.Module):
    ...
    def init_token(self, batch_size, SOS_ID=SOS_ID):
        ...
        x = to_var(torch.LongTensor([SOS_ID] * batch_size), on_cpu=self.cpu)
        return x
    ...
    def embed(self, x):
        ...
        if self.training and self.word_drop > 0.0:
            if random.random() < self.word_drop:
                embed = self.embedding(to_var(x.data.new([UNK_ID] * x.size(0)), on_cpu=self.cpu))
            else:
    ...
    def beam_decode(self,
        ...
        batch_position = to_var(torch.arange(0, batch_size).long() * self.beam_size, on_cpu=self.cpu)
        ...
        score = to_var(score, on_cpu=self.cpu)
...
class DecoderRNN(BaseRNNDecoder):
    def __init__(self, vocab_size, embedding_size,
                 ...
                 max_unroll=30, sample=True, temperature=1.0, beam_size=1, cpu=False):
